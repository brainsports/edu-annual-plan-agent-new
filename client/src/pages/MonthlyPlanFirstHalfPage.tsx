...
const FIRST_HALF_MONTHS = [1, 2, 3, 4, 5, 6];
  ...
  return (
    <Card className="sticky top-24 h-fit max-h-[calc(100vh-8rem)] overflow-y-auto">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg">미리보기 (상반기)</CardTitle>
      </CardHeader>
...
export default function MonthlyPlanFirstHalfPage() {
  ...
  const firstHalfPlans = useMemo(() => {
    return monthlyPlans
      .filter((p) => p.year === year && FIRST_HALF_MONTHS.includes(p.month))
  ...
  const monthDataMap = useMemo(() => {
    ...
    FIRST_HALF_MONTHS.forEach((m) => {
      const plan = firstHalfPlans.find((p) => p.month === m);
  ...
  const handleUpdateMonthData = (month: number, data: MonthData) => {
    const existingPlan = firstHalfPlans.find((p) => p.month === month);
  ...
  const generateMonthMutation = useMutation({
    mutationFn: async (month: number) => {
      const plan = firstHalfPlans.find((p) => p.month === month);
  ...
  const handleExampleFill = () => {
    ...
    toast({
      title: "예시 작성 완료",
      description: "상반기 전체를 예시 데이터로 채웠습니다.",
    });
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1
              ...
              data-testid="heading-first-half"
            >
              월간 사업계획서 (상반기)
            </h1>
