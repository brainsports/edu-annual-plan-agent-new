...
from doc_utils import (generate_part1_report, generate_part2_report,
                       ...
                       generate_monthly_program_report, generate_full_report)
...
if st.session_state.analysis_data is None:
    ...
    with tab2:
        ...
        eval_df = pd.DataFrame(eval_data) if eval_data else pd.DataFrame(
            columns=[
                'program_name', 'eval_tool', 'eval_method', 'eval_timing'
            ])
        ...
        if preview_mode_p2:
            ...
            edited_eval = st.data_editor(
                ...
                column_config={
                    "프로그램명": st.column_config.TextColumn("프로그램명",
                                                         width="medium"),
                    "평가도구": st.column_config.TextColumn("평가도구",
                                                        width="medium"),
    ...
    with tab3:
        ...
        h1_report = generate_monthly_program_report(data['part3_monthly_plan'],
                                                    h1_months)
        st.download_button(
    ...
    with tab4:
        ...
        h2_report = generate_monthly_program_report(data['part4_monthly_plan'],
                                                    h2_months)
        st.download_button(
