...
matplotlib.use('Agg')  # 화면 표시 없이 백그라운드에서 그래프 생성
...
def generate_satisfaction_chart_image(survey_data, total_respondents):
    ...
    # Shorten long questions for the chart
    questions = [
        str(q)[:18] + '...' if len(str(q)) > 18 else str(q)
        for q in df['문항'].tolist()
    ]
...
def generate_part1_report(data_dict: dict) -> io.BytesIO:
    ...
    if satisfaction_survey and satisfaction_survey.get('survey_data'):
        ...
        add_justified_paragraph(document,
                                f"전체 평균 만족도: {overall_avg:.2f}점 (5점 만점)")

        try:
            chart_img = generate_satisfaction_chart_image(
                survey_data, total_respondents)
        ...
        if subjective_q:
            add_left_aligned_heading(document,
                                     f'주관식 문항: {subjective_q}',
                                     level=3)
...
def generate_part2_report(programs_dict: dict) -> io.BytesIO:
    ...
    for category in categories:
        if category in programs_dict:
            ...
            if 'detail_table' in category_data and category_data[
                    'detail_table']:
            ...
            if 'eval_table' in category_data and category_data['eval_table']:
                ...
                if 'sub_area' in df.columns and 'main_plan' in df.columns:
                    ...
                    existing_cols = [
                        c for c in expected_cols if c in df.columns
                    ]
                    df = df[existing_cols]
                    df_to_word_table(document,
                                     df,
                                     '평가계획',
                                     table_type='part2_eval_enhanced')
                else:
                    column_mapping = {
                        'program_name': '프로그램명',
                        'eval_tool': '평가도구',
...
def generate_monthly_program_report(monthly_plan: dict,
                                    months: list = None) -> io.BytesIO:
...
def generate_budget_evaluation_report(budget_eval_data: dict) -> io.BytesIO:
    ...
    if 'budget_table' in budget_eval_data and budget_eval_data['budget_table']:
        ...
        column_mapping = {'category': '항목', 'amount': '금액', 'details': '세부내용'}
        df = df.rename(columns=column_mapping)
    ...
    if 'feedback_summary' in budget_eval_data and budget_eval_data[
            'feedback_summary']:
        add_left_aligned_heading(document, '2. 평가 및 환류 요약', level=2)
        df = pd.DataFrame(budget_eval_data['feedback_summary'])
        column_mapping = {'area': '영역', 'problem': '문제점', 'plan': '개선계획'}
        df = df.rename(columns=column_mapping)
...
def generate_full_report(data_dict: dict) -> io.BytesIO:
    ...
    if 'part4_budget_evaluation' in data_dict and data_dict[
        ...
        if 'feedback_summary' in budget_eval and budget_eval[
                'feedback_summary']:
            add_left_aligned_heading(document, '2. 평가 및 환류 요약', level=2)
